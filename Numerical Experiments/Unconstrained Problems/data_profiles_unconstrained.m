function data_profiles_unconstrained (H, N, gate)
...
colors  = ['r' 'b' "[0 0.5 0]" 'k' "[0.8500 0.3250 0.0980]" 'm'  'y'];   lines   = {'-' '-.' ':' '--' '-' '-.'};
markers = [ '.' '.' '.' '.' '.' '.' 'x' 'h' '+' 'd' '^' '<' ];
...
legend({'TRFD-S','NEWUOA','DFQRM','TRFD-2'},'Location','southeast','Fontsize', 14);
legend boxon

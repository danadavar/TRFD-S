%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Run this file to obtain Data Profiles for Bound Constraints Problems on the Moré-Wild collection
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% NOMAD must be installed. To that end, the user should follow the instructions on the link https://github.com/bbopt/nomad/issues/128,
% which is mentioned on https://nomad-4-user-guide.readthedocs.io/en/latest/Appendix.html#guide-matlab-mex.
% The instructions should be followed by using the folder "nomad-v.4.4.0", which is already provided in "...\Numerical Experiments\Methods\NOMAD"
...
for p = 1:height(N)
    ...
    [~, ~, ~, H3] = run_nomad (x0, Ffun, nfmax, lb, ub, p); 
    [~, ~, ~, ~, H4] = TRFD (x0, Ffun, nfmax, [], [], 0.1, 20, 8);
...
data_profiles_bound_constraints (H, N, 10^(-1));
data_profiles_bound_constraints (H, N, 10^(-3));
data_profiles_bound_constraints (H, N, 10^(-5));
data_profiles_bound_constraints (H, N, 10^(-7));